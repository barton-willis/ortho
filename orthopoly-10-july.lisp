...
;; Define f(k) = 2F1(-k,b,c,x). We will use upward recursion (see DLMF http://dlmf.nist.gov/15.5.E11) to 
;; evaluate f(n-1) along with a pretty good estimate for an upper bound for the rounding error in
;; the computed value of f(n-1). To approximately bound the rounding error

;; The recursion is
;;
;;    f(k+1) = (k * (x-1) /(c+k)) * f(k-1) + ((2*k + c - (b+k)*x)/(c+k))* f(k)
;;
;; Or symbolically
;;    
;;   f(k+1) = a(k) f(k) + b(k) f(k-1).
;;
;; At the end of the first "while" loop, the array fs holds the values f(0), f(1), ... ,f(n-1),
;; the array as holds the values a(0), a(1), ... , a(n-1), and the array holds the values
;; b(0), b(1), ... , b(n-1)
;; 
...
(defmfun $fred (n b c x)
  (hypergeo21-polynomial-numeric n b c x))

(in-package :maxima)

(defun standardize-float-type (&rest x)
  "Standardizes the input arguments and returns them as multiple values."
  (values-list
   (cond ((some #'$bfloatp x) (mapcar #'(lambda (s) (bigfloat::to ($bfloat s))) x))
         ((some #'floatp x)  (mapcar #'(lambda (s) (bigfloat::to ($float s))) x))
         (t                  (mapcar #'bigfloat::to x)))))

(defun hypergeo21-polynomial-numeric (n b c x)
  "Receives multiple values from the bigfloat implementation and formats them as a Maxima list."
  ;; Pass the input arguments b, c, x to get standardized versions bb, cc, xx
  (multiple-value-bind (bb cc xx)
      (standardize-float-type b c x)
    (multiple-value-bind (val err)
        (bigfloat::hypergeo21-polynomial-numeric n bb cc xx)
      ;; Use ftake to build the Maxima list [value, error_bound] cleanly.
      ;; Note: 'err' already includes epsilon scaling from the bigfloat loop.
      (ftake 'mlist (maxima::to val) (maxima::to err)))))
...
variable ~:M" arg (car (last arg))))
  ...
  #| 
(defun hypergeo11-float (n b x)
	(values (ftake '%hypergeometric (ftake 'mlist n) (ftake 'mlist b) x) +flonum-epsilon+))

(defun hypergeo21-float (n b c x)
	(values (ftake '%hypergeometric (ftake 'mlist n b) (ftake 'mlist c) x) +flonum-epsilon+))
|#